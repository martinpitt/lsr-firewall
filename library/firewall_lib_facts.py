...
def offline_cmd(module, *args, defaults=False):
    return module.run_command(
        ...
        (["--system-config=/nonexisting"] if defaults else []) +
        [*args],
        check_rc=True)[1].strip()
...
def config_to_dict(module):
    ...
    if online:
        ...
        for setting in setting_list:
            ...
            if setting == "zones":
                for zone in values:
                    services = offline_cmd(module, "--zone", zone, "--list-services", defaults=True).split()
        ...
        for setting in setting_list:
            ...
            if setting == "zones":
                for zone in values:
                    services = offline_cmd(module, "--zone", zone, "--list-services").split()
